...
- 二刷
    - 还是暴力法，虽然过不了。但是，正确的写出来也需要技巧，以及对于边界的控制。
...
- 三刷
    - 上来先是暴力法，没过。但思路没问题。
    - 2 pointers。
        - 我不知道这个题是不是 2pointers/sliding window的例题，但我是通过这个题学会的sliding window.
        - 2pointers的做法非常多。这个题我觉得思路更多还是来自于sliding window.
        - 双指针，都向前，一个快，一个慢。维护一个sliding window.同时，可以计算其sum
            - tail pointer向后，增加区间的sum.
            - head pointer向后，减少区间的sum.
        - 这个题卡了我两个点
            - 其一，对于min_len的更新时机，在区间减少的循环了更新。因为区间减少，也可能满足target条件，同时可以更新min_len.
...
- 一刷
    - 方法一：空间复杂度O(N)的解法好做，只需要处理好'#'即可，正常插入，遇到这个符号pop_back已经生成的序列即可
    - 方法二：就地删除的办法没有想到。主要是针对消除这个操作应该怎么做？一直没想到
        - 核心操作，遇到'#'，累加消除符号。
        - 否则，只要消除符号，消除即可。就是这一步的操作，一直没想到，特巧妙的，利用了counter做消除。
        - 实现上还有一些细节
            - 外层大循环条件是or，因为可能一个序列一直消除到底，还得让另一个序列消除。
            - 最后判断是否一直，就看两个序列的下标是否都能消除到底。
        - 还有c++的一些实现问题
            - ```if (val)``` means, if val is not zero.
...
- 二刷：二刷的时候还是做得不好，不过debug了一段时间，也算增加了认知。
    - dummy node，tail指针。这两个需要增加，意识到了。
    - dummy node 做的不好的地方在于，对这个dummy node的作用，理解不深刻。
        - dummy node的作用，是避免特殊操作。大部分操作我意识到了。
        - 但是findIndexThNode这个函数没有，因为dummy node的存在。offset从-1开始，这也是增加dummmy node的意义所在。
    - tail_的处理，对于删除，由于没有dummy node。所以该删除，就正常删除。tail_判断的时候放到最后。
        - 这里我做的不好的地方在于，没有掌握好tail处理的时机。
        - 同时，正常删的逻辑，也写错了。
