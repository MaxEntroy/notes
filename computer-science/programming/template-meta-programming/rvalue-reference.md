...
- 但是，形参一定是左值，不管它是一个左值引用，还是右值引用。一个变量是左值还是右值，和它是什么类型没有关系。
  ...
  - 所以，需要一个工具，可以将这个左值形参(左值引用或者右值引用)，**自动保持和其实参的value category一致**，即：
    - 形参是左值，类型是通用引用。如果接受左值实参。```std::forward```转化后，value type是左值引用, value category是xvalue。具备identity属性，可以被左值引用绑定。
    - 形参是左值，类型是通用引用。如果接受右值实参。```std::forward```转化后，value type是右值引用, value category是xvalue。具备movable from属性，可以被右值引用绑定。
